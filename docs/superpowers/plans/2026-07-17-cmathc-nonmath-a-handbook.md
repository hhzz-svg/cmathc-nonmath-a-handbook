...
- `outputs/cmathc_nonmath_a_handbook/src/chapter09.typ`：高等数学综合强化。
...
以“识别结构、选择工具、核对条件”为主线，串联极限与 Taylor、一元微分与积分、常微分方程与空间几何、多元极值、重积分与场论公式、级数与 Fourier 展开。只讲前八章方法之间的组合，不引入初赛范围之外的新科目。

- [ ] **Step 2: 编写 8 道精讲题**

固定题型为：参数极限与高阶展开、微分约束下的积分不等式、含参积分与函数性质、微分方程和积分方程互化、空间几何与多元微分联立、多元最值的边界比较、重积分与场论公式综合、幂级数与 Fourier 展开综合。每题标明方法入口、适用条件和放弃其他方法的理由。

- [ ] **Step 3: 编写 5 道短例、5 道训练和答案**

覆盖换元与缩放、参数分离、对称性、辅助函数、区域分割和端点单独讨论。短例只突出一个选择信号，训练题至少跨两个高等数学模块，答案保留关键条件检查。

- [ ] **Step 4: 独立复算**

检查展开阶数、定理条件、参数分支、微分方程通解遗漏、空间方向、全局极值边界、积分区域与 Jacobian、曲线曲面方向、级数端点和 Fourier 间断点取值。八道精讲题全部登记复核。

- [ ] **Step 5: 编译和范围检查**

Expected: Chapter 9 约 11 页，18 题均有答案锚点；八个高等数学范围模块均至少由一道综合题覆盖，且正文不存在绑定 `S-LA-000` 的题目。

- [ ] **Step 6: 追加任务检查点**

在 `progress.md` 记录八个高等数学模块的综合覆盖、题量、页数和复核结果。
...
写入 40--50 个触发信号、50 个易错点、Taylor 与等价无穷小表、积分公式、级数判别流程、多元微积分与场论公式选择表、证明题句式和考场检查清单。两道精讲题展示跨方法快速选择；2 道短例和 2 道训练验证速查表可直接使用。
...
六题依次覆盖极限与微分、积分、微分方程或空间几何、多元微分与最值、多元积分或场论、级数与高数综合；难度递增，使用原创或经典模型重组，给出建议总用时和分题评分点。
...
题目索引按章节和方法双重组织，列出题号、题型、难度和页码；技巧索引按“极限、证明、积分、微分方程、坐标、多元微积分、级数、综合”分类。索引使用 Typst 引用自动显示页码，避免手写页码失效。
